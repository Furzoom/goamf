...
func TestWriteU29(t *testing.T) {
	for _, c := range testU29Cases {
		...
		n, err := Amf3WriteU29(buf, c.value)
		if err != nil {
...
func TestAMF3_EncodeFloat(t *testing.T) {
	buf := new(bytes.Buffer)
	n, err := Amf3WriteDouble(buf, float64(1.2))
	if err != nil {
...
func TestAMF3_EncodeBoolean(t *testing.T) {
	buf := new(bytes.Buffer)
	n, err := Amf3WriteBoolean(buf, true)
	if err != nil {
	...
	buf = new(bytes.Buffer)
	n, err = Amf3WriteBoolean(buf, false)
	if err != nil {
		t.Fatalf("AMF3_WriteBoolean error: %s", err)
	}
	if n != 1 {
