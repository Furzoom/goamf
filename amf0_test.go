...
func TestWriteMarker(t *testing.T) {
	...
	n, err := WriteMarker(buf, Amf0NumberMarker)
	if err != nil {
		t.Errorf("test for %s error: %s", "WriteMarker", err)
	} else {
		expect := []byte{0x00}
		got := buf.Bytes()
...
func TestEncodeLongString(t *testing.T) {
	...
	if marker != Amf0LongStringMarker {
...
func TestReadMarker(t *testing.T) {
	buf := bytes.NewBuffer([]byte{Amf0NumberMarker})
	marker, err := ReadMarker(buf)
	if err != nil {
		t.Error("ReadMarker err:", err)
	} else {
		if Amf0NumberMarker != marker {
			t.Errorf("ReadMarker: expect %x got %x", Amf0NumberMarker, marker)
		}
	}
}
